  ...
  return (
    <div className='flex flex-col space-x-3 border-y border-gray-100 p-5'>
            <div className='flex space-x-3'>
                <img src={tweet.author.image} alt='profile' className='rounded-full w-10 h-10 object-cover'/>
                <div>
            ...
            <div className='my-2 mt-5 max-h-44 space-y-5 overflow-y-scroll border-t border-gray-100 p-5'>
                {tweet.relatedComments.map((comment) => (
                    <div key={comment._id} className='relative flex space-x-2'>
                            ...
                            <img src={comment.author?.image} alt='profile' className='mt-2 rounded-full w-7 h-7 object-cover'/>
                            <div>
                                <div className='flex items-center space-x-1'>
                                    <p className='mr-1 font-bold'> {comment.author?.name} </p>
                                    <p className='hidden text-sm text-gray-600 md:inline'> @{comment.author?.name.replace(/\s+/g,"").toLowerCase()} </p>
                                    <ReactTimeAgo
